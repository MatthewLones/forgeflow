  ...
  async seedIfEmpty(): Promise<void> {
    ...
    const flow: FlowDefinition = {
      ...
      nodes: [
        ...
        },
        {
          ...
          instructions: [
            ...
            '2. **Competitive landscape** — Map direct and indirect competitors with a CSV comparison matrix',
            '3. **Market dynamics** — Growth trends, regulatory tailwinds/headwinds',
            '4. **Differentiation** — Evaluate moat strength and defensibility',
            '5. **Market map** — Generate an SVG visual showing the competitive positioning landscape',
            '',
            ...
            '',
            'Outputs: \\market_analysis \\competitor_matrix \\market_map',
          ].join('\n'),
          config: {
        ...
        },
        {
          ...
          instructions: [
            'The market research phase is complete. Review the market analysis, competitive matrix, and market positioning map.',
            '',
            'You will see:',
            '- **Market Analysis** (JSON) — TAM/SAM/SOM estimates with supporting data',
            '- **Competitor Matrix** (CSV) — Side-by-side comparison of key competitors',
            '- **Market Map** (SVG) — Visual competitive positioning diagram',
            '',
            'Please provide your feedback as a JSON file with:',
            '- **approved**: true/false — whether the market sizing methodology is sound',
            '- **adjustments**: any corrections to TAM/SAM/SOM estimates',
            '- **missing_competitors**: competitors that should be added to the analysis',
            '- **notes**: additional guidance for the risk assessment phase',
          ].join('\n'),
          config: {
            inputs: ['market_analysis', 'competitor_matrix', 'market_map'],
            outputs: ['market_feedback'],
            skills: [],
            presentation: {
              title: 'Market Research Complete — Validate Sizing & Competitive Landscape',
              sections: [],
            },
          },
          children: [],
        },
        {
          ...
          config: {
            inputs: ['company_profile', 'market_analysis', 'market_feedback', 'financial_findings', 'legal_findings', 'team_assessment'],
            outputs: ['risk_matrix'],
            skills: [],
          ...
          children: [
            {
              ...
              instructions: [
                ...
                '',
                'Also produce a 5-year financial projection model as a CSV spreadsheet with columns for Year, Revenue, Burn Rate, Runway (months), Headcount, and ARR Growth.',
                '',
                'Apply /skill:venture-analysis financial modeling frameworks.',
                '',
                'Outputs: \\financial_findings \\financial_projections',
              ].join('\n'),
              config: {
      ...
      artifacts: {
        ...
        company_profile: {
          ...
          fields: [
            ...
            { key: 'team', type: 'array' as const, description: 'Founding team members with name, role, and background' },
            { key: 'traction', type: 'object' as const, description: 'Traction metrics (ARR, MRR, customers, NRR, retention)' },
            { key: 'funding_history', type: 'array' as const, description: 'Prior funding rounds (date, amount, investors)' },
            { key: 'cap_table', type: 'object' as const, description: 'Current cap table breakdown by shareholder category' },
          ],
        },
        market_analysis: {
          name: 'market_analysis', format: 'json' as const, description: 'TAM/SAM/SOM sizing, competitive landscape, market dynamics',
          fields: [
            { key: 'tam', type: 'object' as const, description: 'Total Addressable Market sizing with value and methodology' },
            { key: 'sam', type: 'object' as const, description: 'Serviceable Available Market with value and methodology' },
            { key: 'som', type: 'object' as const, description: 'Serviceable Obtainable Market with 5-year capture estimate' },
            { key: 'competitors', type: 'array' as const, description: 'Competitive landscape entries with positioning' },
            { key: 'growth_trends', type: 'string' as const, description: 'Market growth trend summary' },
            { key: 'moat_assessment', type: 'object' as const, description: 'Defensibility analysis (data moat, network effects, switching costs)' },
          ],
        },
        competitor_matrix: {
          name: 'competitor_matrix', format: 'csv' as const, description: 'Side-by-side competitor comparison matrix with key metrics',
        },
        market_map: {
          name: 'market_map', format: 'image' as const, description: 'SVG visual showing competitive positioning on capability vs. market focus axes',
        },
        market_feedback: {
          name: 'market_feedback', format: 'json' as const, description: 'Human reviewer feedback on market sizing methodology and competitive landscape',
          fields: [
            { key: 'approved', type: 'boolean' as const, description: 'Whether the market sizing methodology is sound' },
            { key: 'adjustments', type: 'string' as const, description: 'Corrections to TAM/SAM/SOM estimates' },
            { key: 'missing_competitors', type: 'array' as const, description: 'Competitors that should be added', required: false },
            { key: 'notes', type: 'string' as const, description: 'Additional guidance for risk assessment' },
          ],
        },
        financial_findings: {
          name: 'financial_findings', format: 'json' as const, description: 'Unit economics, burn rate, runway, and financial health assessment',
          fields: [
            { key: 'arr', type: 'number' as const, description: 'Annual Recurring Revenue ($)' },
            { key: 'mrr', type: 'number' as const, description: 'Monthly Recurring Revenue ($)' },
            { key: 'burn_rate', type: 'number' as const, description: 'Monthly burn rate ($)' },
            { key: 'runway_months', type: 'number' as const, description: 'Runway at current burn rate' },
            { key: 'ltv_cac_ratio', type: 'number' as const, description: 'LTV:CAC ratio' },
            { key: 'burn_multiple', type: 'number' as const, description: 'Net burn / net new ARR' },
            { key: 'unit_economics', type: 'object' as const, description: 'Detailed unit economics (CAC, LTV, payback period)' },
            { key: 'assessment', type: 'string' as const, description: 'Overall financial health narrative' },
          ],
        },
        financial_projections: {
          name: 'financial_projections', format: 'csv' as const, description: '5-year financial projection model with revenue, burn rate, runway, headcount, and growth',
        },
        legal_findings: {
          name: 'legal_findings', format: 'json' as const, description: 'Cap table review, IP assignment status, regulatory exposure',
          fields: [
            { key: 'cap_table_flags', type: 'array' as const, description: 'Red flags identified in cap table structure' },
            { key: 'ip_status', type: 'object' as const, description: 'IP assignment completeness and patent status' },
            { key: 'regulatory_risks', type: 'array' as const, description: 'Sector-specific regulatory exposures' },
            { key: 'corporate_structure', type: 'string' as const, description: 'Entity type and jurisdiction assessment' },
            { key: 'overall_risk_level', type: 'string' as const, description: 'low, moderate, or high' },
          ],
        },
        team_assessment: {
          name: 'team_assessment', format: 'json' as const, description: 'Team strength, key person risk, gaps, and hiring plan',
          fields: [
            { key: 'founder_market_fit', type: 'number' as const, description: 'Score 1-5 for founder-market fit' },
            { key: 'technical_strength', type: 'number' as const, description: 'Score 1-5 for technical capability' },
            { key: 'key_person_risk', type: 'string' as const, description: 'Assessment of single-point-of-failure risks' },
            { key: 'team_gaps', type: 'array' as const, description: 'Identified hiring gaps' },
            { key: 'culture_signals', type: 'string' as const, description: 'Culture and organizational health indicators' },
          ],
        },
        risk_matrix: {
          ...
          fields: [
            { key: 'financial_risk', type: 'number' as const, description: 'Financial risk score (1=critical, 5=exceptional)' },
            { key: 'legal_risk', type: 'number' as const, description: 'Legal risk score (1=critical, 5=exceptional)' },
            { key: 'team_risk', type: 'number' as const, description: 'Team risk score (1=critical, 5=exceptional)' },
            { key: 'market_risk', type: 'number' as const, description: 'Market risk score (1=critical, 5=exceptional)' },
            { key: 'overall_risk', type: 'number' as const, description: 'Overall risk score (1=critical, 5=exceptional)' },
            { key: 'summary', type: 'string' as const, description: 'Executive summary of key risk factors' },
            { key: 'recommendations', type: 'array' as const, description: 'Specific risk mitigation recommendations' },
          ],
        },
        partner_decisions: {
      ...
      edges: [
        ...
        { from: 'market_review', to: 'risk_assessment' },
