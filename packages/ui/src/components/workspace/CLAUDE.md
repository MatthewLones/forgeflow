...
- `AgentEditor` syncs config from instructions via `extractConfigFromInstructions()` on a 500ms debounce. The config bottom panel reflects parsed state. Never write config sync logic elsewhere.
